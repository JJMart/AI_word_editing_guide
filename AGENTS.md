...
6. **Comment-annotated edit** — any of the above (most often Find/Replace) with an explanatory comment attached to the changed text so the rationale travels with the tracked change. Single-pass pattern: `oDoc.Comments.Add Range:=<found range>, Text:="..."` on the OLD text, then overwrite via `Range.Text = "<new>"`. See "Attaching comments to tracked changes" below.

Full code skeletons for all six patterns are in the "Reference Patterns" section at the bottom of `VBA_MACRO_TEMPLATE.bas`. Copy the relevant pattern into the EDIT BLOCKS region and adapt.
...
### Attaching comments to tracked changes

The writer may ask for an explanatory comment to be attached to a change so the rationale travels with the tracked revision (useful when co-reviewers will see the document). Use the **single-pass find → comment → change** pattern, never a two-phase "edit now, comment after acceptance" design:

1. Find the OLD (current) text and capture it as a `Range`.
2. Attach the comment to that range *before* changing it: `oDoc.Comments.Add Range:=oRng, Text:="<rationale>"`.
3. Overwrite the text: `oRng.Text = "<new text>"` — under `TrackRevisions = True` this records a tracked deletion + insertion, and the comment stays anchored to the deleted (strikethrough) original.

This keeps the old value, new value, and rationale visible together with **no acceptance step required**. Rules:

- **Never anchor a comment to the corrected (new) value and require the writer to "Accept All Changes" first.** The new value only exists as a tracked insertion until accepted; forcing acceptance destroys the audit trail the writer wants to send to reviewers. See the matching entry in `AI_ERRORS_TO_AVOID.md`.
- **Use the `DoEdit` helper sub** in `VBA_MACRO_TEMPLATE.bas` (and the Reference Pattern 6 example that calls it) so each comment-annotated correction stays one logical line and logs `[OK]`/`[FAIL]` like every other edit. Paste the `DoEdit` sub alongside the `ReviewEdits` sub in the delivered macro whenever Pattern 6 is used. A comment-only flag (no text change) reuses the same helper by passing identical find and replace text.
- **Do not walk by occurrence count when the same value must be commented/changed in multiple places.** Under `TrackRevisions = True`, `Range.Text = "..."` leaves the original characters as strikethrough, and `Find` re-matches them on the next iteration — so a hit-counting loop counts each location twice and skips later ones. Anchor each instance with unique surrounding text, or bracket all targets in one Range and use `Replace:=wdReplaceAll`. See the matching entry in `AI_ERRORS_TO_AVOID.md`.
- **Comments live in a separate story range.** `oDoc.Content.Find` does not reach comment text, so comments cannot be edited by a later body-text Find/Replace.
...
- **For a shared document that already contains unaccepted tracked changes from co-authors, ask the writer for two `.md` exports** and read both:
  - **Accepted view** (`--track-changes=accept`) — the clean post-acceptance text. **Use this as the drafting target**: anchor strings, replacement text, and section content come from here because it reflects what the document will read like once revisions are accepted.
  - **All-changes view** (`--track-changes=all`) — shows every co-author insertion (`{++...++}`) and deletion (`{--...--}`) inline. **Use this as a safety check**: before proposing an edit, confirm in this view that the target text has not already been deleted or modified by a co-author. Do not edit text that a co-author has already struck through, and treat text near revision boundaries as fragile (see the `Find`-across-boundary entry in `AI_ERRORS_TO_AVOID.md`).
  - Recommended commands: `pandoc "MyDocument.docx" --wrap=none --track-changes=accept -o "MyDocument_accepted.md"` and `pandoc "MyDocument.docx" --wrap=none --track-changes=all -o "MyDocument_allchanges.md"`. If the writer provides only one export, ask whether the document has pending tracked changes; if it does, request the second export before drafting anchors.
